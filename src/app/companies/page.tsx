  ...
  return (
    <AppShell>
      <div className="px-5 py-5 sm:px-8">
        ...
        <form action={createCompanyAction} className="mt-5 grid gap-4 rounded-md border border-[#2a2a2a] bg-[#111111] p-4 md:grid-cols-3">
          <input className="h-10 rounded-md border border-[#3a3a3a] px-3 text-sm" name="name" placeholder="Company name" required />
          <input className="h-10 rounded-md border border-[#3a3a3a] px-3 text-sm" name="website_url" placeholder="Website URL" />
          <input className="h-10 rounded-md border border-[#3a3a3a] px-3 text-sm" name="industry" placeholder="Industry" />
          ...
          <input className="h-10 rounded-md border border-[#3a3a3a] px-3 text-sm" name="company_size" placeholder="Company size" />
          <textarea className="min-h-24 rounded-md border border-[#3a3a3a] px-3 py-2 text-sm md:col-span-3" name="notes" placeholder="CRM notes, sponsor fit, relationship context" />
        ...
        <div className="mt-5 rounded-md border border-[#2a2a2a] bg-[#111111]">
          ...
          <div className="divide-y divide-[#242424]">
            {companies.length === 0 ? (
              <p className="px-4 py-5 text-sm text-[#a3a3a3]">No companies yet.</p>
            ) : (
              companies.map((company) => (
                <article className="grid gap-3 px-4 py-4 md:grid-cols-[1fr_160px_160px_1fr]" key={String(company.id)}>
                  <div>
                    ...
                    <p className="mt-1 text-sm text-[#a3a3a3]">{String(company.website_url ?? "No website")}</p>
                  </div>
                  <div>
                  ...
                  <p className="text-sm text-[#d4d4d4]">{String(company.notes ?? "No account notes yet.")}</p>
                </article>
              ))
            )}
          </div>
